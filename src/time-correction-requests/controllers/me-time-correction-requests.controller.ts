...
export class MeTimeCorrectionRequestsController {
  ...
  @ApiResponse({ status: 201, type: TimeCorrectionRequestResponseDto })
  async submit(
    @Body() dto: SubmitTimeCorrectionRequestDto,
    @CurrentUser() me: User,
  ): Promise<TimeCorrectionRequestResponseDto> {
    const created = await this.service.submit({ ...dto, employee_id: me.id }, me);
    return TimeCorrectionRequestAssembler.toResponse(created);
  }
  ...
  async getOne(
    ...
    const row = await this.service.findByIdForViewer(id, me);
  ...
  async cancel(
    ...
    const row = await this.service.cancel(id, me);
    return TimeCorrectionRequestAssembler.toResponse(row);
  }
}
