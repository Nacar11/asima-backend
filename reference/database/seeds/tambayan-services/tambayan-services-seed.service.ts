...
@Injectable()
export class TambayanServicesSeedService implements ISeedService {
  constructor(
  ...
  private async uploadSeedImages(): Promise<void> {
    ...
    for (const asset of assets) {
      ...
      try {
        ...
        console.log(
          `✅ Uploaded tambayan-services seed image: ${asset.objectKey}`,
        );
      } catch (error) {
        const err = error as NodeJS.ErrnoException;
        if (err.code === 'ENOENT') {
          console.warn(
            `⚠️  Missing tambayan-services seed image file: ${sourcePath}`,
          );
          continue;
        }
        console.warn(
  ...
  async run(): Promise<void> {
    ...
    if (!owner) {
      console.log(
        '⚠️  owner@cody.inc not found. Skipping tambayan-services seed.',
      );
      return;
    }
    ...
    if (!phpCurrency) {
      console.log(
        '⚠️  PHP currency not found. Skipping tambayan-services seed.',
      );
      return;
    }
    ...
    console.log(
      `✅ Tambayan-services seed completed (created: ${createdCount}, updated: ${updatedCount})`,
    );
  }
  ...
  async down(): Promise<void> {
    ...
    if (services.length === 0) {
      console.log('ℹ️  No tambayan-services to rollback.');
      return;
    }
    ...
    console.log(
      `✅ Tambayan-services rollback completed (${serviceIds.length} services removed)`,
    );
  }
  ...
  private async ensureSellerForService(
    ...
    if (!user) {
      ...
      console.log(
        `✅ Tambayan-services merchant user created (${merchantEmail})`,
      );
    } else {
      user = await this.userRepository.save({
        ...user,
      ...
      console.log(
        `✅ Tambayan-services merchant user updated as store owner (${merchantEmail})`,
      );
    }
    ...
    if (!seller) {
      ...
      console.log(
        `✅ Tambayan-services seller created (${merchantName}) id=${seller.id}`,
      );
    } else {
      seller = await this.sellerRepository.save({
        ...seller,
